//combine Master,task and wave board. JL 9/11/2025
/*
  Pro/Anti task including: 1. Tatile/Delay/Auditory peroid-duration can be set in SD card
                           2. Delay2 can be set in SD card	
						   3. min/max/fininal delay-period-duration and step can be set in SD card
						   4. in proticol-21, Pro/Anti task can be set to block switching OR Randomly Choosing
						   5. optostim
						   6. trial-type percentage can be set in sd card 
*/
...
#define PWM_PERIOD_PIN_53_1 50000 //2khz    
#define PWM_DUTY_PIN_53_1  25000 

#define PWM_PERIOD_PIN_53_2 12500 //8khz   
#define PWM_DUTY_PIN_53_2  6250    

#define PWM_PERIOD_PIN_53_3 6250 //16khz   
#define PWM_DUTY_PIN_53_3  3125 
...
int stim_duration = 1300; //ms
int flat_durCont = 2400;
int ramp_durCont = 200;
int timer4Period = 500;   //us
...
int stimTrial_pct = 27;
int trialType_pct = 75;
...
OutputAction NoOptogeneticTrig   = {"PWM3", 0};

OutputAction Sample_OptoOutput;
OutputAction Delay_OptoOutput;
OutputAction Auditory_OptoOutput;

OutputAction AuditoryCueOutput;
OutputAction AuditoryCueL   = {"PWM4", 255};   
OutputAction AuditoryCueM   = {"PWM5", 255};
OutputAction AuditoryCueH   = {"PWM6", 255};
...
typedef struct {
  ...
  float SamplePeriod        = 0.50; //
  float DelayPeriod         = 0.30; // 0.3->1.3
  float AuditoryPeriod      = 0.50;
...
byte trainAnti_from_begin = 1;  //0 train Anti from protocol 19; 1 train Anti from protocol 4.
byte is_random = 0;    //1 from protocol 21 randomly switch between contingency=0 and 1.
byte is_delay2nd = 0;  //1 there is a 2nd delay period
...
void setup() {
  ...
  Timer4.setPeriod(timer4Period); // every 0.5 ms to update DAC value to be sure 40hz optostim waveform
...
int send_protocol_to_Bpod_and_Run() {
  ...
  switch (S.ProtocolType) {
    ...
    case P_FIXATION: { // Lick and Rectract until Switch Triggered, then increasing headfixation duration gradually
        ...
        switch (S.TrialType) {
          case 0: // right
			...
			AuditoryCueOutput = AuditoryCueM;
            break;
          case 1: // left
            LeftLickAction  = "RewardL";
            RightLickAction = "AnswerPeriod";
			AuditoryCueOutput = AuditoryCueL;
            break;
          case 2: // either
            LeftLickAction  = "RewardL";
            RightLickAction = "RewardR";
        ...
        StateChange DelayPeriod_Cond[1]        = {{"Tup", "Auditory"}};
		StateChange Auditory_Cond[1]           = {{"Tup", "AnswerPeriod"}};
        ...
        OutputAction Auditory_Output[1]        = {AuditoryCueOutput};
		...
		states[3]  = CreateState("Auditory",          S.AuditoryPeriod,         1, Auditory_Cond,        1, Auditory_Output);
    ...
    case P_SAMPLE: { // behavioral protocol 1
        ...
        String ActionAfterAuditory;
        if (S.TrialType == 0 && S.GaveFreeReward.flag_R_water == 1) {
          ActionAfterAuditory = "GiveRightDrop";
        } else if (S.TrialType == 1 && S.GaveFreeReward.flag_L_water == 1) {
          ActionAfterAuditory = "GiveLeftDrop";
        } else {
          ActionAfterAuditory = "AnswerPeriod";
        }
        ...
        StateChange SamplePeriod_Cond[1]       = {{"Tup", "DelayPeriod"}};
        StateChange DelayPeriod_Cond[1]        = {{"Tup", "Auditory"}};
		StateChange Auditory_Cond[1]           = {{"Tup", ActionAfterAuditory}};
    ...
    case P_DELAY: { // behavioural protocol 2
        switch (S.TrialType) {
          ...
          case 0:
			...
			if (contingency == 0) {
				...
				AuditoryCueOutput = AuditoryCueH;
			}
            break;
        }

        if (contingency == 0) {
			MovePole(1);    //Anterior Pole
		} else {
			MovePole(0);   //Posterior  Pole
		}

        // Determine if give free water reward
        String ActionAfterAuditory;		
		ActionAfterAuditory = "Delay2ndPeriod";	
		if (is_delay2nd == 0){		
			ActionAfterAuditory = "AnswerPeriod";	
			if (S.TrialType == 0 && S.GaveFreeReward.flag_R_water == 1) {
				ActionAfterAuditory = "GiveRightDrop";
		...
		String ActionAfterDelay2nd;			
		ActionAfterDelay2nd = "ResponseCue";
		if (S.TrialType == 0 && S.GaveFreeReward.flag_R_water == 1) {
			ActionAfterDelay2nd = "GiveRightDrop2";
		} else if (S.TrialType == 1 && S.GaveFreeReward.flag_L_water == 1) {
			ActionAfterDelay2nd = "GiveLeftDrop2";
		}

        StateChange TrigTrialStart_Cond[1]     = {{"Tup", "SamplePeriod"}}; //
        StateChange SamplePeriod_Cond[1]       = {{"Tup", "DelayPeriod"}};
        StateChange DelayPeriod_Cond[3]        = {{"Port1In", "EarlyLickDelay"}, {"Port2In", "EarlyLickDelay"}, {"Tup", "Auditory"}};
        StateChange EarlyLickDelay_Cond[1]     = {{"Tup", "DelayPeriod"}};
        StateChange Auditory_Cond[1]           = {{"Tup", ActionAfterAuditory}};
		StateChange Delay2ndPeriod_Cond[3]     = {{"Port1In", "EarlyLickDelay2nd"}, {"Port2In", "EarlyLickDelay2nd"}, {"Tup", ActionAfterDelay2nd}};
		StateChange EarlyLickDelay2nd_Cond[1]  = {{"Tup", "Delay2ndPeriod"}};	
        StateChange GiveFreeDrop_Cond[1]       = {{"Tup", "AnswerPeriod"}};
		StateChange GiveFreeDrop2_Cond[1]      = {{"Tup", "ResponseCue"}};
        ...
        states[0]  = CreateState("TrigTrialStart",    0.01,                     1, TrigTrialStart_Cond,     0, NoOutput);
        states[1]  = CreateState("SamplePeriod",      S.SamplePeriod,           1, SamplePeriod_Cond,       1, Sample_Pole_Output);
        states[2]  = CreateState("DelayPeriod",       S.DelayPeriod,            3, DelayPeriod_Cond,        0, NoOutput);
        states[3]  = CreateState("EarlyLickDelay",    0.05,                     1, EarlyLickDelay_Cond,     0, NoOutput);
        states[4]  = CreateState("Auditory",          S.AuditoryPeriod,         1, Auditory_Cond,           1, Auditory_Output);
        states[5]  = CreateState("GiveRightDrop",     Reward_duration_behavior, 1, GiveFreeDrop_Cond,       1, GiveRightDrop_Output);
        states[6]  = CreateState("GiveLeftDrop",      Reward_duration_behavior, 1, GiveFreeDrop_Cond,       1, GiveLeftDrop_Output);
        states[7]  = CreateState("AnswerPeriod",      AnswerPeriod_behavior,    3, AnswerPeriod_Cond,       0, NoOutput);
        states[8]  = CreateState("Reward",            Reward_duration_behavior, 1, Reward_Cond,             1, Reward_Output);
        states[9]  = CreateState("RewardConsumption", ConsumptionPeriod,        1, Tup_StopLicking_Cond,    0, NoOutput);
        states[10] = CreateState("NoResponse",        0.002,                    1, Tup_StopLicking_Cond,    0, NoOutput);
        states[11] = CreateState("TimeOut",           S.TimeOut,                1, Tup_StopLicking_Cond,    0, NoOutput);
		states[12] = CreateState("Delay2ndPeriod",    S.DelayPeriod,            3, Delay2ndPeriod_Cond,     0, NoOutput);
		states[13] = CreateState("EarlyLickDelay2nd", 0.05,                     1, EarlyLickDelay2nd_Cond,  0, NoOutput);
		states[14] = CreateState("GiveRightDrop2",    Reward_duration_behavior, 1, GiveFreeDrop2_Cond,      1, GiveRightDrop_Output);
        states[15] = CreateState("GiveLeftDrop2",     Reward_duration_behavior, 1, GiveFreeDrop2_Cond,      1, GiveLeftDrop_Output);
		states[16] = CreateState("ResponseCue",       0.1,                      1, ResponseCue_Cond,        1, ResponseCue_Output);		
        states[17] = CreateState("StopLicking",       StopLickingPeriod,        3, StopLicking_Cond,        0, NoOutput);
        states[18] = CreateState("StopLickingReturn", 0.01,                     1, Tup_StopLicking_Cond,    0, NoOutput);
        states[19] = CreateState("TrialEnd",          0.01,                     1, TrialEnd_Cond,           0, NoOutput);
    ...
    case P_OPTOSTIM: { // behavioural protocol 3
		...
		// Determine if give free water reward and next step after Auditory
        String ActionAfterAuditory;
		ActionAfterAuditory = "Delay2ndPeriod";			
		if (is_delay2nd == 0){
			ActionAfterAuditory = "AnswerPeriod";
		...
		Sample_OptoOutput = NoOptogeneticTrig;
		Delay_OptoOutput = NoOptogeneticTrig;	
		Auditory_OptoOutput = NoOptogeneticTrig;
		trial_stim_index = 0;
		powerWeight = 0;        
		laserByte = 2; // 1-Thorlab; 2-UltraLaser			
		if (S.ProtocolHistoryIndex == 21  && random(100) < stimTrial_pct) {  
			...
			if (randomNum < 100) {
				Sample_OptoOutput = OptogeneticTrig;
				stim_duration = S.SamplePeriod * 1000;
				trial_stim_index = weightByte + 1; // 11, 51, 101;  stim at sample
			} else if (randomNum < 200) {
				Delay_OptoOutput = OptogeneticTrig;	
				stim_duration = S.DelayPeriod * 1000;
				trial_stim_index = weightByte + 2; // 12, 52, 102; stim at delay
			} else {
				Auditory_OptoOutput = OptogeneticTrig;
				stim_duration = S.AuditoryPeriod * 1000;
				trial_stim_index = weightByte + 3; // 13, 53, 103;   stim at auditory
			}			
		}
        ...
        StateChange DelayPeriod_Cond[3]        = {{"Port1In", "EarlyLickDelay"}, {"Port2In", "EarlyLickDelay"}, {"Tup", "Auditory"}};
        StateChange EarlyLickDelay_Cond[1]     = {{"Tup", "DelayPeriod"}};
	    StateChange Auditory_Cond[3]           = {{"Port1In", "EarlyLickAuditory"}, {"Port2In", "EarlyLickAuditory"}, {"Tup", ActionAfterAuditory}};
		StateChange EarlyLickAuditory_Cond[1]  = {{"Tup", "Auditory"}};
		StateChange Delay2ndPeriod_Cond[3]     = {{"Port1In", "EarlyLickDelay2nd"}, {"Port2In", "EarlyLickDelay2nd"}, {"Tup", ActionAfterDelay2nd}};
        StateChange EarlyLickDelay2nd_Cond[1]  = {{"Tup", "Delay2ndPeriod"}};	
        StateChange GiveFreeDrop_Cond[1]       = {{"Tup", "AnswerPeriod"}};	
		StateChange GiveFreeDrop2_Cond[1]      = {{"Tup", "ResponseCue"}};	
        ...
        OutputAction Sample_Pole_Output[2]     		= {PoleOutput, Sample_OptoOutput}; 
		OutputAction EarlyLick_Sample_Output[1]		= {PoleOutput}; 
		OutputAction delay_Output[1]           		= {Delay_OptoOutput};
        OutputAction Auditory_Output[2]        		= {AuditoryCueOutput,Auditory_OptoOutput};
		OutputAction EarlyLick_Auditory_Output[1]	= {AuditoryCueOutput};
		OutputAction ResponseCue_Output[1]     		= {CueOutput};
		OutputAction GiveRightDrop_Output[1]   		= {RightWaterOutput};
        OutputAction GiveLeftDrop_Output[1]    		= {LeftWaterOutput};
        OutputAction Reward_Output[1]         		= {RewardOutput};
        ...
        states[1]  = CreateState("SamplePeriod",      S.SamplePeriod,           3, SamplePeriod_Cond,        2, Sample_Pole_Output);
        states[2]  = CreateState("EarlyLickSample",   0.05,                     1, EarlyLickSample_Cond,     1, EarlyLick_Sample_Output);
        states[3]  = CreateState("DelayPeriod",       S.DelayPeriod,            3, DelayPeriod_Cond,         1, delay_Output);
        states[4]  = CreateState("EarlyLickDelay",    0.05,                     1, EarlyLickDelay_Cond,      0, NoOutput);
        states[5]  = CreateState("Auditory",          S.AuditoryPeriod,         3, Auditory_Cond,            2, Auditory_Output);
		states[6]  = CreateState("EarlyLickAuditory", 0.05,                     1, EarlyLickAuditory_Cond,   1, EarlyLick_Auditory_Output);
		states[7]  = CreateState("GiveRightDrop",     Reward_duration_behavior, 1, GiveFreeDrop_Cond,        1, GiveRightDrop_Output);
        states[8]  = CreateState("GiveLeftDrop",      Reward_duration_behavior, 1, GiveFreeDrop_Cond,        1, GiveLeftDrop_Output);
        states[9]  = CreateState("AnswerPeriod",      AnswerPeriod_behavior,    3, AnswerPeriod_Cond,        0, NoOutput);
		states[10] = CreateState("Reward",            Reward_duration_behavior, 1, Reward_Cond,              1, Reward_Output);
		states[11] = CreateState("RewardConsumption", ConsumptionPeriod,        1, Tup_StopLicking_Cond,     0, NoOutput);
		states[12] = CreateState("NoResponse",        0.002,                    1, Tup_StopLicking_Cond,     0, NoOutput);
		states[13] = CreateState("TimeOut",           S.TimeOut,                1, Tup_StopLicking_Cond,     0, NoOutput);
		states[14] = CreateState("Delay2ndPeriod",    S.DelayPeriod,            3, Delay2ndPeriod_Cond,      0, NoOutput);
        states[15] = CreateState("EarlyLickDelay2nd", 0.05,                     1, EarlyLickDelay2nd_Cond,   0, NoOutput);
        states[16] = CreateState("GiveRightDrop2",    Reward_duration_behavior, 1, GiveFreeDrop2_Cond,       1, GiveRightDrop_Output);
        states[17] = CreateState("GiveLeftDrop2",     Reward_duration_behavior, 1, GiveFreeDrop2_Cond,       1, GiveLeftDrop_Output);
        states[18] = CreateState("ResponseCue",       0.1,                      1, ResponseCue_Cond,         1, ResponseCue_Output);		
		states[19] = CreateState("StopLicking",       StopLickingPeriod,        3, StopLicking_Cond,         0, NoOutput);
		states[20] = CreateState("StopLickingReturn", 0.01,                     1, Tup_StopLicking_Cond,     0, NoOutput);
		states[21] = CreateState("TrialEnd",          0.05,                     1, TrialEnd_Cond,            0, NoOutput);    
...
int write_SD_eventST_updateOutcome() {
  ...
  switch (S.ProtocolType) {
    ...
    case P_DELAY:
      for (int i = 0; i < nTransition; i++) {
        if (state_visited[i] == 8 || state_visited[i] == 10 || state_visited[i] == 11) { // Reward || No Response || Time Out (error)
          if (state_visited[i] == 8) {
            S.Trial_Outcome = 1;
          } else if (state_visited[i] == 10) {
            S.Trial_Outcome = 0;
          } else {
            S.Trial_Outcome = 2;
          }
      ...
      is_earlylick = 0;
      for (int i = 0; i < nTransition; i++) {
        if (state_visited[i] == 3 || state_visited[i] == 13) { // earlylick delay 1&2
          is_earlylick = 1;
          break;
        }
      }
    ...
    case P_OPTOSTIM: // the same as P_DELAY
      for (int i = 0; i < nTransition; i++) {
        if (state_visited[i] == 10 || state_visited[i] == 12 || state_visited[i] == 13) { // Reward || No Response || Time Out (error)
          if (state_visited[i] == 10) {
            S.Trial_Outcome = 1;
          } else if (state_visited[i] == 12) {
            S.Trial_Outcome = 0;
          } else {
            S.Trial_Outcome = 2;
          }
      ...
      is_earlylick = 0;
      for (int i = 0; i < nTransition; i++) {
        if (state_visited[i] == 2 || state_visited[i] == 4 || state_visited[i] == 6 || state_visited[i] == 15) { // earlylick sample || delay1 || Audittory || delay2
          is_earlylick = 1;
          break;
        }
      }
...
int autoChangeProtocol() {
   ...
   if (S.ProtocolHistoryIndex == 0){
    ...
    //case 1: // P_SAMPLE-1
	if (S.ProtocolHistoryIndex == 1){
      if (S.ProtocolHistory[S.ProtocolHistoryIndex].nTrials > 100) { 
    ...
    //case 19: // P_OPTOSTIM-19:  Pro context
	if (S.ProtocolHistoryIndex == 19){
	  if (S.ProtocolHistory[S.ProtocolHistoryIndex].nTrials > contingency_trial_aim && Perf100 > Perf100_aim && Earlylick100 < Earlylick100_aim && response_trial > response_trial_aim) {
		contingency = 1;
		if (trainAnti_from_begin){  //train Anti context from protocol 4 (1st delay protocol)
			S.ProtocolHistoryIndex = 4;
			S.ProtocolHistory[S.ProtocolHistoryIndex].Protocol = P_DELAY;
			S.ProtocolHistory[S.ProtocolHistoryIndex].nTrials = 0;
			S.ProtocolHistory[S.ProtocolHistoryIndex].performance = 0;
	...
	//case 20: // P_OPTOSTIM-20:Anti context
	if (S.ProtocolHistoryIndex == 20){
		if (S.ProtocolHistory[S.ProtocolHistoryIndex].nTrials > contingency_trial_aim && Perf100 > Perf100_aim && Earlylick100 < Earlylick100_aim && response_trial > response_trial_aim) {
			S.ProtocolHistoryIndex = 21;
			S.ProtocolHistory[S.ProtocolHistoryIndex].Protocol = P_OPTOSTIM;
			S.ProtocolHistory[S.ProtocolHistoryIndex].nTrials = 0;
			S.ProtocolHistory[S.ProtocolHistoryIndex].performance = 0;
			//S.ProtocolType  = P_OPTOSTIM;
	...
	//case 21: // P_OPTOSTIM-21:Pro/Anti task with block switching OR Randomly Choose
	if (S.ProtocolHistoryIndex == 21){
		...
		if (is_random == 1) {  
			...
			if (fixblock_trialnum_cont >= fixblock_trialnum_aim && Perf100 > Perf100_aim && response_trial > response_trial_aim) {
				fixblock_trialnum_cont = 0;
				contingency_trial = 0;
				if (contingency == 1) {
					contingency = 0;
				...
				if (SerialUSB.dtr() && SerialUSB_connected()) {
					SerialUSB.print("M: contingency(fixblock)changed to: ");
...
int write_SD_para_S() {
  ...
  if (dataFile) {
	...
	dataFile.print("AuditoryPeriod = ");
    dataFile.println(S.AuditoryPeriod);
	...
	dataFile.print("trainAnti_from_begin = ");
    dataFile.println(trainAnti_from_begin); 
	
	dataFile.print("is_random = ");
    dataFile.println(is_random); 
	
	dataFile.print("is_delay2nd = ");
    dataFile.println(is_delay2nd); 
	
	dataFile.print("stimTrial_pct = ");
    dataFile.println(stimTrial_pct);
	
	dataFile.print("trialType_pct = ");
    dataFile.println(trialType_pct);
...
int read_SD_para_S() {
  ...
  if (dataFile) {
	...
	buffer_tmp = dataFile.readStringUntil('=');
    buffer_tmp = dataFile.readStringUntil('\n');
    S.AuditoryPeriod = buffer_tmp.toFloat();	
    buffer_tmp = dataFile.readStringUntil('=');
    buffer_tmp = dataFile.readStringUntil('\n');
    ...
    buffer_tmp = dataFile.readStringUntil('=');
    buffer_tmp = dataFile.readStringUntil('\n');
    trainAnti_from_begin = buffer_tmp.toInt(); 
	...
	buffer_tmp = dataFile.readStringUntil('=');
    buffer_tmp = dataFile.readStringUntil('\n');
    trialType_pct = buffer_tmp.toInt();	
...
void OptoStim() {
	...
	flat_durCont = 1000*stim_duration/timer4Period-ramp_durCont;
    if (laserByte == 1) {
...
void timer_updateDAC() { // total 1.3 sec stim by default
	sample_ind ++;
	if (laserByte == 1) { // Thorlab laser
		if (sample_ind < flat_durCont) { // flat cos, 1.2 sec by default
			dac.setVoltage(uint16_t(powerWeight * thorlab_flat_sine[sample_ind % 50]), false);
		} else if (sample_ind < (flat_durCont+ramp_durCont)) { // ramp down, 0.1 sec by default
			dac.setVoltage(uint16_t(powerWeight * thorlab_ramp_sine[sample_ind - flat_durCont]), false);
		} else {
			Timer4.stop();
			dac.setVoltage(0, false);
		}
	}else{
		if (sample_ind < flat_durCont) { // flat cos, 1.2 sec
			dac.setVoltage(uint16_t(powerWeight * (ultra_flat_sine[sample_ind % 50] - 655) + 655), false); // 655 is 0.8 V
		}else if (sample_ind < (flat_durCont+ramp_durCont)) { // ramp down, 0.1 sec
			dac.setVoltage(uint16_t(powerWeight * (ultra_ramp_sine[sample_ind - flat_durCont] - 655) + 655), false); // 655 is 0.8 V
		}else {
			Timer4.stop();
			dac.setVoltage(0, false);
		}
...
void setStateOutputs(byte State) {
	...
	if (OutputStateMatrix[State][9] == 255) {
		...
		pwm_pin53.start(PWM_PERIOD_PIN_53_1, PWM_DUTY_PIN_53_1);
	}else if (OutputStateMatrix[State][13] == 255) {
		pwm_pin53.start(PWM_PERIOD_PIN_53_2, PWM_DUTY_PIN_53_2);
	}else if (OutputStateMatrix[State][14] == 255) {
		pwm_pin53.start(PWM_PERIOD_PIN_53_3, PWM_DUTY_PIN_53_3);
	}else {
		stopGocue();
	}
