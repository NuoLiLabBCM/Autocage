...
int regulatorVal_max     = 215;//255; // max pressure
...
OutputAction AuditoryCueL   = {"PWM2", 255};
OutputAction AuditoryCueM   = {"PWM3", 255};
OutputAction AuditoryCueH   = {"PWM4", 255};
...
typedef struct {
  ...
  float SamplePeriod        = 0.5; //
  float DelayPeriod         = 0.30; // 0.3->1.3
  float AuditoryPeriod      = 0.5;
...
int sampleType = 1;  //0 for Tactile; 1 for Auditory.
...
int send_protocol_to_Bpod_and_Run() {
  ...
  switch (S.ProtocolType) {
    ...
    case P_FIXATION: { // Lick and Rectract until Switch Triggered, then increasing headfixation duration gradually
		...
		MovePole(1); // 2 sec, Anterior pole		
    ...
    case P_OPTOSTIM: { // behavioural protocol 3
        ...
        states[5]  = CreateState("Auditory",          S.AuditoryPeriod,         3, Auditory_Cond,            1, Auditory_Output);
		states[6]  = CreateState("EarlyLickAuditory", 0.05,                     1, EarlyLickAuditory_Cond,   1, Auditory_Output);
  ...
  switch (S.Autolearn) {
    ...
    case 2: { // Autolearn     'antiBias'
        ...
        if (switch_indicator == MaxSame) {
          ...
          //randomSeed(analogRead(0));
		  //JH changed to adjust fraction of trials
		  if (contingency == 0) {
			if (random(100) <= 75) {
				S.TrialType = 0;
			} else {
