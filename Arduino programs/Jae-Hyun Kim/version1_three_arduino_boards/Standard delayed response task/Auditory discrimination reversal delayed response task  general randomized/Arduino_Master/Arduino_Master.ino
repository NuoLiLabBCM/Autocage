...
OutputAction SampleCueOutputH   = {"PWM2", 255};
OutputAction SampleCueOutputL   = {"PWM3", 255};
...
int send_protocol_to_Bpod_and_Run() {
  ...
  switch (S.ProtocolType) {
    ...
    case P_FIXATION: { // Lick and Rectract until Switch Triggered, then increasing headfixation duration gradually
        ...
        OutputAction Sample_Pole_Output[1]     = {SampleCueOutput}; //{PoleOutput};
    ...
    case P_OPTOSTIM: { // behavioural protocol 3
        ...
        OutputAction TrigTrialStart_Output[1]  = {WaveSurferTrig};
        OutputAction Sample_Pole_Output[1]     = {SampleCueOutput};//{PoleOutput};
       // OutputAction Sample_Pole_Stim_Output[2] = {SampleCueOutput, OptogeneticTrig};//{PoleOutput, OptogeneticTrig};
        OutputAction ResponseCue_Output[1]     = {CueOutput};
       // OutputAction ResponseCueStim_Output[2] = {CueOutput, OptogeneticTrig};
        ...
        if (reverse_num > 2 && optostim_num == 1) {  // starting optostim from 3rd reversal	 
          weightByte = 10; // percent power level: 100%
		  ...
		  states[9]  = CreateState("Reward",            Reward_duration_behavior, 1, Reward_Cond,          2, RewardStim_Output);
		  states[10] = CreateState("RewardConsumption", ConsumptionPeriod,        1, Tup_StopLicking_Cond, 1, OptoStim_Output);
		  states[11] = CreateState("NoResponse",        0.002,                    1, Tup_StopLicking_Cond, 1, OptoStim_Output);
		  states[12] = CreateState("TimeOut",           S.TimeOut,                1, Tup_StopLicking_Cond, 1, OptoStim_Output);
		  states[13] = CreateState("StopLicking",       StopLickingPeriod,        3, StopLicking_Cond,     1, OptoStim_Output);
		  states[14] = CreateState("StopLickingReturn", 0.01,                     1, Tup_StopLicking_Cond, 1, OptoStim_Output);
  ...
  switch (S.ProtocolHistoryIndex) {
    ...
    case 19: // P_OPTOSTIM-19:
	  ...
	  if (SerialUSB.dtr() && SerialUSB_connected()) {
		...
		// 2/4/2026 JH added (randomization of modality)
	  ...
	  if (meetaim_cont < 2) {
		...
		if (fixblock_trialnum_cont >= fixblock_trialnum_aim && Perf100 > Perf100_aim && Earlylick100 < Earlylick100_aim && response_trial > response_trial_aim ) {
			...
			if (SerialUSB.dtr() && SerialUSB_connected()) {
				SerialUSB.print("M: sampleType(fixblock)changed to: ");
				SerialUSB.println(sampleType);
			}
		}
	  }
