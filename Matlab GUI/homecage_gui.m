...
handles.hbutton_reward = uicontrol('Style','pushbutton','String','Set&Reward',...
    ...
    'Position',[0.07,0.7155,0.04,0.02]);

% weight, Tare
handles.htext_tare = uicontrol('Style','text','String','Weight (g)',...
    'Units', 'normalized',        'Tag', 'text_tare',...
    'Position',[0.01,0.668,0.03,0.02],'HorizontalAlignment', 'left');
handles.hedit_tare = uicontrol('Style','edit','String','0',...
    'Units', 'normalized',        'Tag', 'edit_tare',...
    'Position',[0.04,0.673,0.025,0.02]);
handles.hbutton_tare = uicontrol('Style','pushbutton','String','Tare',...
    'Units', 'normalized',        'Tag', 'button_tare',...
    'Callback',{@button_moveSet,'Tare'},...
    'Position',[0.07,0.673,0.04,0.02]);

% weight, Calibration
handles.htext_cali = uicontrol('Style','text','String','Known_W (g)',...
    'Units', 'normalized',        'Tag', 'text_cali',...
    'Position',[0.01,0.64,0.04,0.02],'HorizontalAlignment', 'left');
handles.hedit_cali = uicontrol('Style','edit','String','0',...
    'Units', 'normalized',        'Tag', 'edit_cali',...
    'Position',[0.05,0.645,0.015,0.02]);
handles.hbutton_cali = uicontrol('Style','pushbutton','String','Calibration',...
    'Units', 'normalized',        'Tag', 'button_cali',...
    'Callback',{@button_moveSet,'CaliF'},...
    ...
    function button_moveSet(~, ~, arg)
        ...
        switch arg
            ...
            case 'Tare'
             ...
             case 'CaliF'
                selection = questdlg('Make sure an object with Known Weight is on the plateform and the weight value is input in the edit field!!!',...
                    'Warning',...
                    'Yes','No','No');
                switch selection
                    case 'Yes'
                        write(handles.s{Cage_num},'G','char'); % Command
                        writeline(handles.s{Cage_num},get(handles.hedit_cali,'String'));
                        msgbox('Done!');
                    case 'No'
                        return;
                end
            case 'Struggle' 
                ...
                msgbox('Done!');
            case 'Polepos'
                write(handles.s{Cage_num},'E','char'); % Command
                writeline(handles.s{Cage_num},get(handles.hedit_poleposA,'String'));
                writeline(handles.s{Cage_num},get(handles.hedit_poleposP,'String'));
                msgbox('Done!');
            otherwise
...
%                  disp('Weight-end');
            ...
            case 'C' % control panel info
                try
                    ...
                    set(handles.hedit_poleposA, 'String', A(ind_semicolon(9)+1:ind_semicolon(10)-1));
                    set(handles.hedit_poleposP, 'String', A(ind_semicolon(10)+1:ind_semicolon(11)-1));
                catch e
